                ...
                NavHost(
                    ...
                    composable("home") {
                        HomeScreen(
                            onNavigateToEditor = { name, type ->
                                navController.navigate("editor/${Uri.encode(name)}/${Uri.encode(type)}")
                            },
                    ...
                    composable(
                        ...
                        EditorScreen(
                            ...
                            onNavigateToEditor = { newName, newType ->
                                navController.navigate("editor/${Uri.encode(newName)}/${Uri.encode(newType)}")
                            },
                    ...
                    composable("versions") {
                        VersionDocumentsScreen(
                            onNavigateToHistory = { fileName ->
                                navController.navigate("versions/${Uri.encode(fileName)}")
                            },
                            onNavigateBack = { navController.popBackStack() },
                            onNavigateToHome = {
                                navController.navigate("home") {
                    ...
                    composable(
                        ...
                        VersionsScreen(
                            ...
                            onViewVersion = { versionId ->
                                navController.navigate("version_preview/${Uri.encode(versionId)}?fileName=${Uri.encode(fileName)}")
                            },
                            onCompareVersions = { v1, v2 ->
                                navController.navigate("diff/${Uri.encode(v2)}?oldVersionId=${Uri.encode(v1 ?: "")}&fileName=${Uri.encode(fileName)}")
                            },
                            onNavigateToHome = {
                                navController.navigate("home") {
                                    popUpTo("home") { inclusive = true }
                    ...
                    composable(
                        ...
                        VersionPreviewScreen(
                            ...
                            onRestore = {
                                navController.navigate("versions/${Uri.encode(fileName)}") {
                                    popUpTo("versions/${Uri.encode(fileName)}") { inclusive = true }
                                }
                            }
                        )
                    }
